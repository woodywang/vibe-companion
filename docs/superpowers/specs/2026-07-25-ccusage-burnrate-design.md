...
| # | 项 | ccusage | 本实现 | 理由 |
|---|---|---|---|---|
| D1 | 空闲行为 | 速率冻结至 5h 块失活 | 距末条 entry > 90s 归零 | 常驻仪表需要"熄火"反馈；CLI 跑完即退无此需求 |
...
**分母是「首条 entry → 末条 entry」**，不是块起点，也不是 `now`。空闲时间不会稀释速率——这是 D1 偏离存在的原因。
...
| 无速率 | 活跃块仅 1 条 entry（`duration <= 0`） | 指针归零，LCD 显示 `--`（**不是 `0`**） |
| 空闲 | 距末条 entry > 90s | 指针缓慢落回 0，隐藏气泡 |
